...
/*
async function loadQuestionsButtons() {
    const pokemonIds = [];
    while(pokemonIds.length < 4) {
     ...
     pokemons.forEach(pokemon => {
       ...
       button.className = "btn-nombre";
       button.textContent = pokemon.name;
       contenedorBotones.appendChild(button);
     })
     temporizador();
     mostrarDatos(pokemonIds[0]);
}
...
function mostrarDatos(idPokemon) {
    const idImagePokemon = getImageId(idPokemon);
    ...
    getPokemon(idPokemon).then(data => {
        ...
        if(data) {
            ...
            infoPokemon.innerHTML = `
                <h3 class="dato">---DESCRIPTION---</h3>
                <p class="dato">Name: ${data.name}</p>
                <p class="dato">Weight: ${data.weight}</p>
                <p class="dato">height: ${data.height}</p>
                <p class="dato">Type: ${data.types.map(typeInfo => typeInfo.type.name).join(', ')}</p>
                <p class="dato">Abilities: ${data.abilities.map(abilitiesInfo => abilitiesInfo.ability.name).join(',')}</p>
            `;

            imagePokemon.innerHTML = `
                <img class="hide" alt="${data.name}" src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/${idImagePokemon}.png">
            `;
        }
    })
...
function temporizador() {
    temp = setInterval(() => {
        contador -= 1;
        contenedorScore.innerHTML = `
            <p class="tiempo">Tiempo: ${contador}s</p>
        `;
        if(contador === 0) {
            clearInterval(temp);
        }
